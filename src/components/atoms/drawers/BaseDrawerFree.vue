...
transition(:name='`drawer-${direction}`')
  .base-drawer(:class='[direction]', v-if='isActive')
    .-header(v-show='withHeader')
      Heading(:number='number', :fontSize='textSize') {{ title }}
...
const DRAWER_OVERLAY_CLASS = 'base-drawer-overlay-re-search'
...
export default class BaseDrawerFree extends Vue {
  ...
  @Prop({ default: 'normal' }) padding?: 'none' | 'normal' | 'wide'
