...
.suggestbox
  vue-simple-suggest(
    ...
    template(slot='misc-item-above')
      paragraph.topStation(fontSize='small') エリア
    .listing(slot='suggestion-item', slot-scope='{suggestion}') {{ suggestion.title }}
      //- train-icon#-trainicon
  //-search-icon#-icon(v-if='$window.width > 768')
  reverce-icon#reverce-icon
</template>
<script lang="ts">
...
import ReverceIcon from '~/assets/icon/reverce-triangle.svg'
