...
export default class NewsIndex extends Vue {
  ...
  get dataInit() {
    ...
    let data = this.$store.getters['modules/notification/getDetailTO'](num)
    if (!data) {
      this.$store.dispatch('modules/notification/InitNotificationTO').then(() => {
        data = this.$store.getters['modules/notification/getDetailTO'](num)
        if (!data) {
          return this.$router.push('/takeout')
        }
        return data
      })
    } else {
